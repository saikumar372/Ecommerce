...
class Authentication
{
	...
	*/
	...
	public function is_admin(){
		...
		return $this->check_group($user_id,'Admin');
	}
	...
	*/
	public function is_member(){
		$user_id = $this->getUserId();
		return $this->check_group($user_id,'Member');
	}
	...
	*/
	...
	public function check_group($id,$name){
		$group_id  = $this->db->select('group_id')->where('group_name',$name)->get('groups')->row_array();
		if(!empty($group_id)){
			$res = $this->db->select('user_group_id')->where('user_id',$id)->where('group_id',$group_id['group_id'])->get('user_groups')->row_array();
			if(!empty($res)){
				return 1;
			}else{
				return 0;
